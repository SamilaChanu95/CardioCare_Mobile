...
public class SampleBCNurse extends BroadcastReceiver {
    static int noOfTimes = 0;

    // Method gets called when Broad Case is issued from NurseActivity for every 10 seconds
    @Override
    public void onReceive(final Context context, Intent intent) {
        ...
        client.post("http://192.168.1.101/cardiocare_mobile/nurse/getdbrowcount.php",params ,new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(String response) {
                ...
                try {
                    ...
                    if(obj.getInt("count") != 0){
                        final Intent intnt = new Intent(context, MyServiceNurse.class);
                        // Set unsynced count in intent data
                        intnt.putExtra("intntdata", "There are "+obj.getInt("count") + " unsynced Nurse Details");
                        // Call MyService
