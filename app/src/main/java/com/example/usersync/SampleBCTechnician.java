...
public class SampleBCTechnician extends BroadcastReceiver {
    static int noOfTimes = 0;

    // Method gets called when Broad Case is issued from TechnicianActivity for every 10 seconds
    @Override
    public void onReceive(final Context context, Intent intent) {
        ...
        client.post("http://192.168.1.101/cardiocare_mobile/technician/getdbrowcount.php",params ,new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(String response) {
                ...
                try {
                    ...
                    if(obj.getInt("count") != 0){
                        final Intent intnt = new Intent(context, MyServiceTechnician.class);
                        // Set unsynced count in intent data
                        intnt.putExtra("intntdata", "There are "+obj.getInt("count") + " unsynced Technician Details");
                        // Call MyService
